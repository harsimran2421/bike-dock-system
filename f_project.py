...
class initial_page(QtWidgets.QDialog):
    def __init__(self, parent= None):
        ...
        self.unlock_button.clicked.connect(self.unlock_buttonc) 
...
class Login_rfid(QtWidgets.QDialog):
    ...
    def login_compare(self):
        ...
        for i in items:
            ...
            if i['RFID_value'] != rfid_value:
                QtWidgets.QMessageBox.warning(self,'Error','Invalid Entry')
                sys.exit(app.exec_())       
            elif i['slot'] == '0' and i['RFID_value'] == rfid_value:
                test_flag = 1
                self.accept()
                break
    ...
    def on_unlock_buttonc_clicked(self):
        ...
        for i in items:
            if i['RFID_value'] != rfid_value:
                ...
                QtWidgets.QMessageBox.warning(self,'Error','Slot not booked')
                sys.exit(app.exec_())       
                break
            else:
                print("Entered else loop")
                slot_value = i['slot']
                self.status_label.setText("Dock open for " + str(timer_val) + " sec")
                sleep(timer_val)
...
if Initial.exec_() == QtWidgets.QDialog.Accepted and username_login_flag == 1:
    login = Login()
    if login.exec_() == QtWidgets.QDialog.Accepted:
        if test_flag == 1:
            ...
            if w.exec_() == QtWidgets.QDialog.Accepted:
                sys.exit(app.exec_())
elif Initial.exec_() == QtWidgets.QDialog.Accepted and rfid_login_flag == 1:
    login = Login_rfid()
    if login.exec_() == QtWidgets.QDialog.Accepted:
        if test_flag == 1:
            w = project()
...
elif Initial.exec_() == QtWidgets.QDialog.Accepted:
    login = unlock_slot()
    if login.exec_() == QtWidgets.QDialog.Accepted:
        sys.exit(app.exec_())
